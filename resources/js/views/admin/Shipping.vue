<template>
  <div>
    <v-data-table :headers="headers" :items="shipping" sort-by="id" class="elevation-1">
      <template v-slot:top>
        <v-toolbar flat color="white">
          <v-toolbar-title>shipping Methods</v-toolbar-title>
...
export default {
  ...
  methods: {
    ...
    deleteItem(item) {
      ...
      if (decide) {
        axios
          .delete("/api/admin/shipping/" + item.id)
          .then(res => {
            this.snackbar = true;
    ...
    save() {
      if (this.editedIndex > -1) {
        axios.put("/api/admin/shipping/" + this.editedItem.id, this.editedItem);
        ...
        axios
          .post("/api/admin/shipping", this.editedItem)
          .then(res => this.shipping.push(res.data.shipping))
